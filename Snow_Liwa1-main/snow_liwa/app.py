...
# Dynamically import settings_utils after BASE_DIR is defined
settings_utils_path = BASE_DIR / "settings_utils.py"
spec = importlib.util.spec_from_file_location(
    "settings_utils", str(settings_utils_path))
settings_utils = importlib.util.module_from_spec(spec)
sys.modules["settings_utils"] = settings_utils
spec.loader.exec_module(settings_utils)
load_settings = settings_utils.load_settings
...
def render_customer_info_panel():
    ...
    st.markdown('<div class="section-card" style="margin-top:1.5rem;">',
                unsafe_allow_html=True)
...
def render_welcome():
    ...
    hero_intro_paragraph = settings.get(
    ...
    with col_left:
        ...
        with st.form("booking_form"):
            ...
            tickets = st.number_input(
                "Number of tickets / عدد التذاكر", 1, 20, 1)
            notes = st.text_area(
                "Notes (optional) / ملاحظات اختيارية", height=70)
    ...
    with col_right:
        st.markdown('<div class="ticket-card ticket-card-poster">',
                    unsafe_allow_html=True)
        if poster_path and os.path.exists(poster_path):
            ...
            st.write(
                "No poster image configured yet. Please upload one from the Settings page.")
        st.markdown("</div>", unsafe_allow_html=True)
...
def render_payment_result(result: str, pi_id: str):
    ...
    st.link_button("Back to SNOW LIWA home",
                   get_ziina_app_base_url(), use_container_width=False)
    st.markdown("</div>", unsafe_allow_html=True)
