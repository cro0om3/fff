...
def has_ziina_configured() -> bool:
    ...
    if not token:
        return False
    # Treat common placeholder patterns as not configured
    if isinstance(token, str) and (token.startswith("PUT_") or token.startswith("REPLACE_") or token.upper().startswith("TEST")):
        return False
    return True
...
def render_payment_result(result: str, pi_id: str):
    ...
    # Try exact match first, then fallback to contains (handles truncated/encoded ids)
    ...
    if pi_id_norm:
        ...
        if pi:
            # Ziina response shape may vary; try common paths for status and id
            ...
            # If we got a returned id that's different or more canonical, normalize it
            if returned_id:
                returned_id = str(returned_id)
                if returned_id != pi_id_norm:
                    pi_id_norm = returned_id

            # Update booking if we found it (use boolean mask to avoid index typing issues)
            if not row.empty:
...
def render_settings():
    ...
    if poster_file:
        ...
        from utility.settings_utils import save_settings
...
def main():
    ...
    # Debug/diagnostics area (moved to admin-only sections)
